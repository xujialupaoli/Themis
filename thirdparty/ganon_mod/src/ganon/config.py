...
class Config:
    ...
    def __init__(self, which: str=None, **kwargs):
        ...
        classify_group_postrep.add_argument("--report-type",            type=str,                    metavar="", default="abundance", help="Type of report (.tre) [" + ", ".join(self.choices_report_type) + "]. More info in 'Report'.", choices=self.choices_report_type)
        classify_group_postrep.add_argument("--skip-report",            action="store_true",                                          help="Disable tree-like report (.tre) at the end of classification. Can be done later with 'Report'.")
        ...
        table_group_required.add_argument("-i", "--input",           type=str,          required=True, nargs="*",     metavar="", help="Input file(s) and/or folder(s). '.tre' file(s) from Report.")
