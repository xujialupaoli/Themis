    ...
    if cfg.db_prefix:
        ...
        if cfg.report_type in ["abundance", "corr"]:
            try:
                ...
                print_log(
                    "Failed to get genome sizes from .tax files, run Report without -d/--db-prefix")
                return False
    else:
...
def correct_genome_size(target_counts, genome_sizes, tax, default_ranks):
    ...
    if no_genome_size_cnt > 0 and len(target_counts) != no_genome_size_cnt:
        print_log(" - WARNING: " + str(no_genome_size_cnt) + " genomes without proper genome size, abundance estimation may be biased. Use a --report-type without genome size correction or omit --db-prefix on Report to re-generate genome sizes.")
